...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'Messmagic', 'Messmagic', and 'Messmagic'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.Messmagic/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.Messmagic/g),
   ...
   * Rename files.
   *
   * Rename files that had Messmagic in their name, such as
   * css/Messmagic.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*Messmagic*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('Messmagic', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-Messmagic-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Messmagic's files so that we can process them.
    glob('*', {ignore: 'node_modules/**' }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace Messmagic name mentions in files.
      generator.replaceFileContents();
      // Rename files that had Messmagic in their name.
